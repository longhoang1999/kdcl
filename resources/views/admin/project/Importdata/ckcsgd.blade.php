...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.ckcsgd')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.ckcsgd.exportCkcsgd') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_csgd_ctgd" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.tddgn')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.kqdgcn')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.nqchd')
                </th>
                <th style="width: 12%;">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUpdateLabel">
                    @lang('project/ImportdataExcel/title.cnckcsgd')
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.ckcsgd.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input type="date" class="form-control " id="fortddgn" placeholder="@lang('project/ImportdataExcel/title.tddgn')" name="tddgn">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forkqdgcn">
                                    <span>@lang('project/ImportdataExcel/title.kqdgcn')</span>
                                </label>
                                <input type="text" class="form-control " id="forkqdgcn" placeholder="@lang('project/ImportdataExcel/title.kqdgcn')" name="kqdgcn">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fornqchd">
                                    <span>@lang('project/ImportdataExcel/title.nqchd')</span>
                                </label>
                                <input type="text" class="form-control " id="fornqchd" placeholder="@lang('project/ImportdataExcel/title.nqchd')" name="nqchd">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="forcnclgd" >
                                    <span>@lang('project/ImportdataExcel/title.cnclgd')</span>
                                </label>
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input type="date" class="form-control " id="forngaycap" placeholder="@lang('project/ImportdataExcel/title.ngaycap')" name="ngaycap">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forgiatriden">
                                    <span>@lang('project/ImportdataExcel/title.giatriden')</span>
                                </label>
                                <input type="date" class="form-control " id="forgiatriden" placeholder="@lang('project/ImportdataExcel/title.giatriden')" name="giatriden">
                            </div>
...
<script>
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.ckcsgd.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
                ...
                var thead = `
                        <thead class="btn-success ">
                            <tr class="border ">
                                ...
                                <th class="row_width p-2" rowspan="2">
                                    @lang('project/ImportdataExcel/title.cnclgd')
                                </th>
                                <th class="row_width p-2" colspan="2">
                                    @lang('project/ImportdataExcel/title.gcncn')
                                </th>
                                <th class="row_width p-2" rowspan="2">
                            ...
                            <tr class="border ">
                                ...
                                <th class="row_width p-2" >
                                    @lang('project/ImportdataExcel/title.giatriden')
                                </th>
                            </tr>
                        </thead>
    ...
    $("#import_unit_data").click(function() {
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    'tcsdt' :   $(this).find('.row1').text().trim(),
                    'tddgn' :  $(this).find('.row2').text().trim(),
                    'kqdgcn' :  $(this).find('.row3').text().trim(),
                    'nqchd' : $(this).find('.row4').text().trim(),
                    'cnclgd' : $(this).find('.row5').text().trim(),
                    'ngaycap' : $(this).find('.row6').text().trim(),
                    'giatriden' : $(this).find('.row7').text().trim(),
                    
                }
                dataSubmit.push(dataObj);
            });

            let loadData = "{{ route('admin.importdata.ckcsgd.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.ckcsgd.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.ckcsgd.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((response) => response.json())
            .then((data) => {
                $("#fortcsdt").val(data.ten_co_so);
                $("#fortddgn").val(data.tddgn);
                $("#forkqdgcn").val(data.ket_qua);
                $("#fornqchd").val(data.nghi_quyet);
                $("#forcnclgd").val(data.cong_nhan);
                $("#forngaycap").val(data.ngay_cap);
                $("#forgiatriden").val(data.gia_tri_den);
