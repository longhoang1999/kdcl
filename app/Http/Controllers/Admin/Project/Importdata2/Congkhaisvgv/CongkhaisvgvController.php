<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Congkhaisvgv;
...
class CongkhaisvgvController extends DefinedController{
    public function index(){
		...
		$donvi = DB::table("donvi")->select("id", "ten_donvi", "deleted_at","loai_dv_id")
                ...
                ->get();
        $getFile = DB::table('excel_import_data2')->where('type_excel', '31')->select("id", "year")->get();
            ...
            if($phanquen){
                if($phanquen->donvi_id == Sentinel::getUser()->donvi_id){
                    if(Carbon::now() > $phanquen->ngay_bd  && Carbon::now() < $phanquen->ngay_kt){
                        return view('admin.project.Importdata2.cksvgv')->with([
                            'loai_dv'           => $loai_dv,
                            'donvi'             => $donvi,
                            'getFile'           => $getFile
                        ]);
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '31')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Congkhaisvgv'), $picName);

            $data = [
                'type_excel'    => '31',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Congkhaisvgv/'.$picName,
