...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.sangche')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.sangche.exportUnit') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_sangche" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.tpmsc')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.cshcn')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.cshdv')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.socongnhan')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.namcap')
                </th>
                <th>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUnitLabel">
                    @lang('project/ImportdataExcel/title.ttsc')
                </h5>
...
<div class="modal fade" id="modalUpdate" tabindex="-1" role="dialog" aria-labelledby="modalUpdateLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.sangche.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label for="fortpmsc">
                                    <span>@lang('project/ImportdataExcel/title.tpmsc')</span>
                                </label>
                                <input type="text" class="form-control " id="fortpmsc" placeholder="@lang('project/ImportdataExcel/title.tpmsc')" name="tpmsc">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="formaso">
                            ...
                            <div class="form-group col-md-3">
                                <label for="forcshcn">
                                    <span>@lang('project/ImportdataExcel/title.cshcn')</span>
                                </label>
                                <input type="text" class="form-control " id="forcshcn" placeholder="@lang('project/ImportdataExcel/title.cshcn')" name="cshcn">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forcshdv">
                                    <span>@lang('project/ImportdataExcel/title.cshdv')</span>
                                </label>
                                <input type="text" class="form-control " id="forcshdv" placeholder="@lang('project/ImportdataExcel/title.cshdv')" name="cshdv">
                            </div>
                            <div class="form-group col-md-3">
                            ...
                            <div class="form-group col-md-3">
                                <label for="forsocongnhan">
                                    <span>@lang('project/ImportdataExcel/title.socongnhan')</span>
                                </label>
                                <input type="text" class="form-control " id="forsocongnhan" placeholder="@lang('project/ImportdataExcel/title.socongnhan')" name="socongnhan">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fornamcap">
                                    <span>@lang('project/ImportdataExcel/title.namcap')</span>
                                </label>
                                <input type="number" class="form-control " id="fornamcap" placeholder="@lang('project/ImportdataExcel/title.namcap')" name="namcap">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fornoicap">
                                    <span>@lang('project/ImportdataExcel/title.noicap')</span>
                                </label>
                                <input type="text" class="form-control " id="fornoicap" placeholder="@lang('project/ImportdataExcel/title.noicap')" name="noicap">
                            </div>
                        </div>
                    </div> 
                </form>
...
<script>
    ...
    $(function(){
        table = $('#table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{!! route('admin.importdata.sangche.dataUnit') !!}",
            columns: [
                { data: 'stt', name: 'stt' ,className: 'stt'},
                { data: 'tpmsc', name: 'tpmsc' },
                { data: 'cshcn', name: 'cshcn' },
                { data: 'cshdv', name: 'cshdv' },
                { data: 'scn', name: 'scn' },
                { data: 'namcap', name: 'namcap' },
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.sangche.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
                ...
                var thead = `
                        <thead class="btn-success ">
                            <tr class="border ">
                                ...
                                <th class="row_width p-2">
                                    @lang('project/ImportdataExcel/title.noicap')
                                </th>
                                <th class="row_width p-2">
        ...
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    ...
                    'tpmsc' :   $(this).find('.row1').text().trim(),
                    'maso' :  $(this).find('.row2').text().trim(),
                    'loai' :   $(this).find('.row3').text().trim(),
                    'tacgia' :  $(this).find('.row4').text().trim(),
                    'cshcn' :  $(this).find('.row5').text().trim(),
                    'cshdv' : $(this).find('.row6').text().trim(),
                    'linhvuc' : $(this).find('.row7').text().trim(),
                    'socongnhan' : $(this).find('.row8').text().trim(),
                    'namcap' : $(this).find('.row9').text().trim(),
                    'noicap' : $(this).find('.row10').text().trim(),
                    
                }
                dataSubmit.push(dataObj);
            });

            let loadData = "{{ route('admin.importdata.sangche.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.sangche.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.sangche.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((response) => response.json())
            .then((data) => {
                $('#fortpmsc').val(data.tpmsc);
                $('#formaso').val(data.maso);
                $('#forloai').val(data.loai);
                $('#fortacgia').val(data.tacgia);
                $('#forcshcn').val(data.cshcn);
                $('#forcshdv').val(data.cshdv);
                $('#forlinhvuc').val(data.linhvuc);
                $('#forsocongnhan').val(data.scn);
                $('#fornamcap').val(data.namcap);
                $('#fornoicap').val(data.noicap);
            })
    })
