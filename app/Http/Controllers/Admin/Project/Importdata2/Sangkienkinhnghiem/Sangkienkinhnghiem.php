<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Sangkienkinhnghiem;
...
class Sangkienkinhnghiem extends DefinedController{
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '9')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Sangkienkinhnghiem'), $picName);

            $data = [
                'type_excel'    => '9',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Sangkienkinhnghiem/'.$picName,
