...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.dtkhcn2')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.dtkhcn2.exportDtkhcn2') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_dtkhcn2" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.dttkhcn')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.tldt')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.tsdt')
                </th>
                <th>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUpdateLabel">
                    @lang('project/ImportdataExcel/title.cndtkhcn2')
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.dtkhcn2.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                                <div class="form-group col-md-3" style="margin-top: 40px">
                                    ...
                                    <input type="number" class="form-control " id="fornam" placeholder="" name="nam">
                                </div>
                                <div class="form-group col-md-3" style="margin-top: 20px">
                                    <label for="fordttkhcn">
                                        <span>@lang('project/ImportdataExcel/title.dttkhcn')</span>
                                    </label>
                                    <input type="number" class="form-control " id="fordttkhcn" placeholder="" name="dttkhcn">
                                </div>
        
                                <div class="form-group col-md-3">
                                    <label for="fortldt">
                                        <span>@lang('project/ImportdataExcel/title.tldt')</span>
                                    </label>
                                    <input type="number" class="form-control " id="fortldt" placeholder="" name="tldt">
                                </div>
                                <div class="form-group col-md-3">
                                    <label for="fortsdt">
                                        <span>@lang('project/ImportdataExcel/title.tsdt')</span>
                                    </label>
                                    <input type="number" class="form-control " id="fortsdt" placeholder="" name="tsdt">
                                </div>
...
<script>
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.dtkhcn2.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
    ...
    $("#import_unit_data").click(function() {
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    'nam' :   $(this).find('.row1').text().trim(),
                    'dttkhcn' :  $(this).find('.row2').text().trim(),
                    'tldt' :   $(this).find('.row3').text().trim(),
                    'tsdt' :  $(this).find('.row4').text().trim(),
                    
                }
                dataSubmit.push(dataObj);
            });

            let loadData = "{{ route('admin.importdata.dtkhcn2.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.dtkhcn2.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((data) => {
                $("#fornam").val(data.nam);
                $("#fordttkhcn").val(data.doanh_thu);
                $("#fortldt").val(data.ty_le_doanh_thu);
                $("#fortsdt").val(data.ty_so_doanh_thu);
