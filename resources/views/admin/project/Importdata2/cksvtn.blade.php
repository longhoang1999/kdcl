...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.cksvtn')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.cksvtn.exportCksvtn') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_svtn_cvl" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.ssvtn')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.loaixs')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.loaig')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.loaikha')
                </th>
                <th style="width: 12%;">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUpdateLabel">
                    @lang('project/ImportdataExcel/title.cncksvtn')
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.cksvtn.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                            ...
                            <div class="form-group col-md-3">
                                <label for="forssvtn">
                                    <span>@lang('project/ImportdataExcel/title.ssvtn')</span>
                                </label>
                                <input type="number" class="form-control " id="forssvtn" placeholder="@lang('project/ImportdataExcel/title.ssvtn')" name="ssvtn">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forloaixs">
                                    <span>@lang('project/ImportdataExcel/title.loaixs')</span>
                                </label>
                                <input type="number" class="form-control " id="forloaixs" placeholder="@lang('project/ImportdataExcel/title.loaixs')" name="loaixs">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forloaig">
                                    <span>@lang('project/ImportdataExcel/title.loaig')</span>
                                </label>
                                <input type="number" class="form-control " id="forloaig" placeholder="@lang('project/ImportdataExcel/title.loaig')" name="loaig">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forloaikha" style="margin-top: 1.65rem;">
                                    <span>@lang('project/ImportdataExcel/title.loaikha')</span>
                                </label>
                                <input type="number" class="form-control " id="forloaikha" placeholder="@lang('project/ImportdataExcel/title.loaikha')" name="loaikha">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fortlsncvl">
                                    <span>@lang('project/ImportdataExcel/title.tlsncvl')</span>
                                </label>
                                <input type="number" class="form-control " id="fortlsncvl" placeholder="@lang('project/ImportdataExcel/title.tlsncvl')" name="tlsncvl">
                            </div>
...
<script>
    ...
    $(function(){
        table = $('#table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{!! route('admin.importdata.cksvtn.dataUnit') !!}",
            columns: [
                { data: 'stt', name: 'stt' ,className: 'stt'},
                { data: 'khoi_nganh', name: 'khoi_nganh' },
                { data: 'ssvtn', name: 'ssvtn' },
                { data: 'xuat_sac', name: 'xuat_sac' },
                { data: 'gioi', name: 'gioi' },
                { data: 'kha', name: 'kha' },
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.cksvtn.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
                        ...
                        <thead class="btn-success ">
                            <tr class="border ">
                                ...
                                <th class="row_width p-2" colspan="3">
                                    @lang('project/ImportdataExcel/title.pltn')
                                </th>
                                <th class="row_width p-2 row_add" rowspan="2">
                                    @lang('project/ImportdataExcel/title.tlsncvl')
                                </th>
                                <th class="row_width p-2 row_add" rowspan="2">
        ...
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    ...
                    'ssvtn' :  $(this).find('.row2').text().trim(),
                    'loaixs' :  $(this).find('.row3').text().trim(),
                    'loaig' : $(this).find('.row4').text().trim(),
                    'loaikha' : $(this).find('.row5').text().trim(),
                    'tlsncvl' : $(this).find('.row6').text().trim(),
                    
                }
                dataSubmit.push(dataObj);
            });

            let loadData = "{{ route('admin.importdata.cksvtn.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.cksvtn.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.cksvtn.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((data) => {
                ...
                $("#forssvtn").val(data.ssvtn);
                $("#forloaixs").val(data.xuat_sac);
                $("#forloaig").val(data.gioi);
                $("#forloaikha").val(data.kha);
                $("#fortlsncvl").val(data.ty_le);
