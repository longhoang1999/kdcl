...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.ckttdt')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.ckttdt.exportCkttdt') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_tt_dao_tao" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.kqdt')
                </th>
                <th style="width: 12%;">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUpdateLabel">
                    @lang('project/ImportdataExcel/title.cnckttdt')
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.ckttdt.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    <input type="hidden" id="id_unit" name="id_unit">
                    <div class="container-fuild">
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label for="fortendvdh">
                                    <span>@lang('project/ImportdataExcel/title.tendvdh')</span>
                                </label>
                                <input type="text" class="form-control " id="fortendvdh" placeholder="@lang('project/ImportdataExcel/title.tendvdh')" name="tendvdh">
                            </div>
                            ...
                            <div class="form-group col-md-4" >
                                <label for="forcndtao">
                                    <span>@lang('project/ImportdataExcel/title.cndtao')</span>
                                </label>
                                <input type="text" class="form-control " id="forcndtao" placeholder="@lang('project/ImportdataExcel/title.cndtao')" name="cndtao">
                            </div>
                            <div class="form-group col-md-3" >
                                <label for="forkqdt">
                                    <span>@lang('project/ImportdataExcel/title.kqdt')</span>
                                </label>
                                <input type="text" class="form-control " id="forkqdt" placeholder="@lang('project/ImportdataExcel/title.kqdt')" name="kqdt">
                            </div>
...
<script>
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.ckttdt.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
                ...
                var thead = `
                        <thead class="btn-success ">
                            <tr class="border ">
                                ...
                                <th class="row_width p-2">
                                    @lang('project/ImportdataExcel/title.cndtao')                                  
                                </th>
                                <th class="row_width p-2 row_add">
                                    @lang('project/ImportdataExcel/title.kqdt')                                  
                                </th>
    ...
    $("#import_unit_data").click(function() {
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    'tendvdh' :   $(this).find('.row1').text().trim(),
                    'soluong' :  $(this).find('.row2').text().trim(),
                    'tddt' :  $(this).find('.row3').text().trim(),
                    'cndtao' :  $(this).find('.row4').text().trim(),
                    'kqdt' :  $(this).find('.row5').text().trim(),
            ...
            let loadData = "{{ route('admin.importdata.ckttdt.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.ckttdt.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.ckttdt.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((response) => response.json())
            .then((data) => {
                $("#fortendvdh").val(data.ten_don_vi);
                $("#forsoluong").val(data.so_luong);
                $("#fortddt").val(data.tddt);
                $("#forcndtao").val(data.cndt);
                $("#forkqdt").val(data.ket_qua);
