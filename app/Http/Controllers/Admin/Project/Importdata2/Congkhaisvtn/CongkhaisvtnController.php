<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Congkhaisvtn;
...
class CongkhaisvtnController extends DefinedController{
    public function index(){
		$loai_dv = DB::table("loai_donvi")->select("id", "loai_donvi")->get();
		$donvi = DB::table("donvi")->select("id", "ten_donvi", "deleted_at","loai_dv_id")
                ->where("deleted_at", null)
                ->get();
        $getFile = DB::table('excel_import_data2')->where('type_excel', '29')->select("id", "year")->get();
		
        return view('admin.project.Importdata2.cksvtn')->with([
            'loai_dv'           => $loai_dv,
            'donvi'             => $donvi,
            'getFile'           => $getFile
        ]);
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '29')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Congkhaisvtn'), $picName);

            $data = [
                'type_excel'    => '29',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Congkhaisvtn/'.$picName,
