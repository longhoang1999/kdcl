<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Congkhaittvhl;
...
//Import Excel
use App\Imports\Congkhaittvhl;

// export excel
use App\Exports\CongkhaittvhlExport;

class CongkhaittvhlController extends DefinedController{
    public function index(){
		$loai_dv = DB::table("loai_donvi")->select("id", "loai_donvi")->get();
		$donvi = DB::table("donvi")->select("id", "ten_donvi", "deleted_at","loai_dv_id")
                ->where("deleted_at", null)
                ->get();
        $getFile = DB::table('excel_import_data2')->where('type_excel', '35')->select("id", "year")->get();
        ...
        if(Sentinel::inRole('truongdonvi')){
            ...
            if($phanquen){
                if($phanquen->donvi_id == Sentinel::getUser()->donvi_id){
                    return view('admin.project.Importdata2.ckttvhl')->with([
                        'loai_dv'           => $loai_dv,
                        'donvi'             => $donvi,
                        'getFile'           => $getFile
                    ]);
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '35')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Congkhaittvhl'), $picName);

            $data = [
                'type_excel'    => '35',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Congkhaittvhl/'.$picName,
    ...
    public function importDataUnit(Request $req) {
        ...
        foreach($data as $dt){
            if($dt->khoinganh != "" && $dt->ssvtn != ""){
                ...
                DB::table("excel_import_hoclieu_tv_tthl")->insert($dataInport);
            }
        }
        $respon = [
            'mes'   => 'done'
    ...
    public function exportCksvgv() {
        return Excel::download(new CongkhaittvhlExport, 'cong-khai-tt-ve-hoc-lieu.xlsx');
    }


    public function dataUnit(Request $req){
    	$donviExcel = DB::table("excel_import_hoclieu_tv_tthl AS cksvgv");
        if(isset($req->id) && $req->id != ''){
            $donviExcel = $donviExcel->where("cksvgv.id", $req->id)->first();
            return json_encode($donviExcel);
        }else{
	        $donviExcel = $donviExcel
	                ->select('cksvgv.id', 'cksvgv.ten', 'cksvgv.so_luong'
	                 );
    ...
    public function deleteUnit(Request $req){
        DB::table('excel_import_hoclieu_tv_tthl')->where("id", $req->id_delete)->delete();
        return back()->with('success', 
    ...
    public function updateUnit(Request $req){
        ...
        DB::table("excel_import_hoclieu_tv_tthl")->where("id", $req->id_unit)
        		->update($data);
        return back()->with('success', 
