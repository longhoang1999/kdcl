...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.skkn')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.sangkienkinhnghiem.exportUnit') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_sangkienkn" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.tensk')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.chunhiem')
                </th>
                <th>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.diemdg')
                </th>
                <th>
...
<div class="modal fade" id="modalUpdate" tabindex="-1" role="dialog" aria-labelledby="modalUpdateLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.sangkienkinhnghiem.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label for="fortensk">
                                    <span>@lang('project/ImportdataExcel/title.tensk')</span>
                                </label>
                                <input type="text" class="form-control " id="fortensk" placeholder="@lang('project/ImportdataExcel/title.tensk')" name="tensk">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forchunhiem">
                                    <span>@lang('project/ImportdataExcel/title.chunhiem')</span>
                                </label>
                                <input type="text" class="form-control " id="forchunhiem" placeholder="@lang('project/ImportdataExcel/title.chunhiem')" name="chunhiem">
                            </div>
                            <div class="form-group col-md-3">
                            ...
                            <div class="form-group col-md-3">
                                <label for="fordiemdg">
                                    <span>@lang('project/ImportdataExcel/title.diemdg')</span>
                                </label>
                                <input type="text" class="form-control " id="fordiemdg" placeholder="@lang('project/ImportdataExcel/title.diemdg')" name="diemdg">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fortgpb">
                                    <span>@lang('project/ImportdataExcel/title.tgpb')</span>
                                </label>
                                <input type="text" class="form-control " id="fortgpb" placeholder="@lang('project/ImportdataExcel/title.tgpb')" name="tgpb">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forghichu">
...
<script>
    ...
    $(function(){
        table = $('#table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{!! route('admin.importdata.sangkienkinhnghiem.dataUnit') !!}",
            columns: [
                { data: 'stt', name: 'stt' ,className: 'stt'},
                { data: 'tensk', name: 'tensk' },
                { data: 'chunhiem', name: 'chunhiem' },
                { data: 'thanhvien', name: 'thanhvien' },
                { data: 'tgnt', name: 'tgnt' },
                { data: 'diemdg', name: 'diemdg' },
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.sangkienkinhnghiem.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
                ...
                var thead = `
                        <thead class="btn-success ">
                            <tr class="border ">
                                ...
                                <th class="row_width p-2">
                                    @lang('project/ImportdataExcel/title.tgpb')
                                </th>
                                <th class="row_width p-2">
        ...
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    ...
                    'tensk' :   $(this).find('.row1').text().trim(),
                    'chunhiem' :   $(this).find('.row2').text().trim(),
                    'thanhvien' :  $(this).find('.row3').text().trim(),

                    'dvct' :   $(this).find('.row4').find('select').val(),
                    'tgnt' :  $(this).find('.row5').text().trim(),
                    'diemdg' :  $(this).find('.row6').text().trim(),
                    'tgpb' : $(this).find('.row7').text().trim(),
                    'ghichu' :   $(this).find('.row8').text().trim(),  
                }
                dataSubmit.push(dataObj);
            });
            let loadData = "{{ route('admin.importdata.sangkienkinhnghiem.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.sangkienkinhnghiem.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.sangkienkinhnghiem.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((response) => response.json())
            .then((data) => {
                $("#fortensk").val(data.tensk);
                $("#forchunhiem").val(data.chunhiem);
                $("#forthanhvien").val(data.thanhvien);
                $("#fordvct").val(data.dvct);
                $("#fortgnt").val(data.tgnt);
                $("#fordiemdg").val(data.diemdg);
                $("#fortgpb").val(data.tgpb);
