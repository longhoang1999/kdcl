...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.gttltk')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <a href="{{ route('admin.importdata.ckgttl.exportCkgttl') }}" class="btn btn-benchmark mr-2" type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
            </a>
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_giaotrinh" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.kehoachst')
                </th>
                <th>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUpdateLabel">
                    @lang('project/ImportdataExcel/title.cngttltk')
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.ckgttl.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                            ...
                            <div class="form-group col-md-3" >
                                ...
                                <input type="number" class="form-control " id="fornamxb" placeholder="@lang('project/ImportdataExcel/title.namxb')" name="namxb">
                            </div>
                            <div class="form-group col-md-4">
                                <label for="forkehoachst">
                                    <span>@lang('project/ImportdataExcel/title.kehoachst')</span>
                                </label>
                                <input type="text" class="form-control " id="forkehoachst" placeholder="@lang('project/ImportdataExcel/title.kehoachst')" name="kehoachst">
                            </div>
...
<script>
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.ckgttl.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
    ...
    $("#import_unit_data").click(function() {
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            let loadData = "{{ route('admin.importdata.ckgttl.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.ckgttl.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.ckgttl.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
