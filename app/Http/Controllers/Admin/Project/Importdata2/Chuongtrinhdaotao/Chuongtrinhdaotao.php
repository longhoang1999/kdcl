<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Chuongtrinhdaotao;
...
class Chuongtrinhdaotao extends DefinedController{
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '3')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Chuongtrinhdaotao'), $picName);

            $data = [
                'type_excel'    => '3',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Chuongtrinhdaotao/'.$picName,
