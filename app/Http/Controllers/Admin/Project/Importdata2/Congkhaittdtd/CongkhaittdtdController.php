<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Congkhaittdtd;
...
//Import Excel
use App\Imports\Ckttdtd;

// export excel
use App\Exports\CkttdtdExport;
...
class CongkhaittdtdController extends DefinedController{
    public function index(){
		...
		$donvi = DB::table("donvi")->select("id", "ten_donvi", "deleted_at","loai_dv_id")
                ...
                ->get();
        $getFile = DB::table('excel_import_data2')->where('type_excel', '34')->select("id", "year")->get();
        ...
        if(Sentinel::inRole('truongdonvi')){
            ...
            if($phanquen){
                if($phanquen->donvi_id == Sentinel::getUser()->donvi_id){
                    return view('admin.project.Importdata2.ckttdtd')->with([
                        'loai_dv'           => $loai_dv,
                        'donvi'             => $donvi,
                        'getFile'           => $getFile
                    ]);
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '34')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Congkhaittdtd'), $picName);

            $data = [
                'type_excel'    => '34',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Congkhaittdtd/'.$picName,
    ...
    public function exportCkttdtd() {
        return Excel::download(new CkttdtdExport, 'Cong-khai-thong-tin-dien-tich-dat.xlsx');
    }


    public function dataUnit(Request $req){
    	$donviExcel = DB::table("excel_import_dtd_dtxxd AS ckttdtd");
        if(isset($req->id) && $req->id != ''){
            $donviExcel = $donviExcel->where("ckttdtd.id", $req->id)->first();
            return json_encode($donviExcel);
        }else{
	        $donviExcel = $donviExcel
	                ->select('ckttdtd.id', 'ckttdtd.ten_du_an', 'ckttdtd.nct_tvtg'
                    , 'ckttdtd.dttn_qt', 'ckttdtd.tgth', 'ckttdtd.kinh_phi', 'ckttdtd.tom_tat');
    ...
    public function deleteUnit(Request $req){
        DB::table('excel_import_dtd_dtxxd')->where("id", $req->id_delete)->delete();
        return back()->with('success', 
