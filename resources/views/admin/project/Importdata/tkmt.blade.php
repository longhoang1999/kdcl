...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.tkmt')
@parent
@stop
...
<section class="content-body">
    <div class="form-standard">
        <div class="item-group-button right-block mb-2">
            ...
            <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_tk_mt" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
        ...
        <table class="table table-striped table-bordered" id="table" width="100%">
            <thead>
             <tr>
                ...
                <th>
                    @lang('project/ImportdataExcel/title.smm')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.smc')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.dcvp')
                </th>
                <th>
                    @lang('project/ImportdataExcel/title.dcht')
                </th>
                <th>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalUpdateLabel">
                    @lang('project/ImportdataExcel/title.cntttkmt')
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.importdata.tkmt.updateUnit') }}" method="post" id="update-unit">
                    @csrf
                    ...
                    <div class="container-fuild">
                        <div class="row">
                            ...
                            <div class="form-group col-md-3">
                                <label for="fortongso">
                                    <span>@lang('project/ImportdataExcel/title.tongso')</span>
                                </label>
                                <input type="number" class="form-control " id="fortongso" placeholder="@lang('project/ImportdataExcel/title.tongso')" name="tongso">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forsmm">
                                    <span>@lang('project/ImportdataExcel/title.smm')</span>
                                </label>
                                <input type="number" class="form-control " id="forsmm" placeholder="@lang('project/ImportdataExcel/title.smm')" name="smm">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forsmc">
                                    <span>@lang('project/ImportdataExcel/title.smc')</span>
                                </label>
                                <input type="number" class="form-control " id="forsmc" placeholder="@lang('project/ImportdataExcel/title.smc')" name="smc">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fordcvp">
                                    <span>@lang('project/ImportdataExcel/title.dcvp')</span>
                                </label>
                                <input type="number" class="form-control " id="fordcvp" placeholder="@lang('project/ImportdataExcel/title.dcvp')" name="dcvp">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="fordcht">
                                    <span>@lang('project/ImportdataExcel/title.dcht')</span>
                                </label>
                                <input type="number" class="form-control " id="fordcht" placeholder="@lang('project/ImportdataExcel/title.dcht')" name="dcht">
                            </div>
                            <div class="form-group col-md-3">
                                <label for="forghichu">
...
<script>
    ...
    $(function(){
        table = $('#table').DataTable({
            ...
            columns: [
                ...
                { data: 'so_may_cu', name: 'so_may_cu' },
                { data: 'dchtvp', name: 'dchtvp' },
                { data: 'dcht', name: 'dcht' },
    ...
    $('#import_unit').on('click', function () {
        ...
        $.ajax({
            url : "{!! route('admin.importdata.tkmt.importUnit') !!}",
            type : 'POST',
            data : formData,
            processData: false,  
            contentType: false,  
        ...
        if(checkEmpty() && checkWebsite() && checkEmail() && checkPhone()
            ...
            $(".row_number").each(function( index ) {
                let dataObj = {
                    ...
                    'dcvp' :  $(this).find('.row5').text().trim(),
                    'dcht' : $(this).find('.row6').text().trim(),
                    'ghichu' : $(this).find('.row7').text().trim(),
                    
                }
                dataSubmit.push(dataObj);
            });

            let loadData = "{{ route('admin.importdata.tkmt.importDataUnit') }}";
            fetch(loadData, {
                headers: {
    ...
    $('#modalDelete').on('show.bs.modal', function (event) {
        ...
        var route = "{{ route('admin.importdata.tkmt.deleteUnit') }}" + "?id_delete=" + id;
        var modal = $(this);
    ...
    $('#modalUpdate').on('show.bs.modal', function (event) {
        ...
        var id = button.data('id') 
        $("#id_unit").val(id);
        let loadData = "{{ route('admin.importdata.tkmt.dataUnit') }}" + "?id=" + id;
        fetch(loadData, {
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            .then((response) => response.json())
            .then((data) => {
                $("#fordonvi").val(data.don_vi);
                $("#fortongso").val(data.tong_so);
                $("#forsmm").val(data.so_may_moi);
                $("#forsmc").val(data.so_may_cu);
                $("#fordcvp").val(data.dchtvp);
                $("#fordcht").val(data.dcht);
                $("#forghichu").val(data.ghi_chu);
