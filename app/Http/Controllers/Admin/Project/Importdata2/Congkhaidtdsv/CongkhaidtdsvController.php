<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Congkhaidtdsv;
...
//Import Excel
use App\Imports\Ckdtdsv;

// export excel
use App\Exports\CkdtdsvExport;

class CongkhaidtdsvController extends DefinedController{
    public function index(){
		$loai_dv = DB::table("loai_donvi")->select("id", "loai_donvi")->get();
		$donvi = DB::table("donvi")->select("id", "ten_donvi", "deleted_at","loai_dv_id")
                ->where("deleted_at", null)
                ->get();
        $getFile = DB::table('excel_import_data2')->where('type_excel', '32')->select("id", "year")->get();
            ...
            if($phanquen){
                if($phanquen->donvi_id == Sentinel::getUser()->donvi_id){
                    if(Carbon::now() > $phanquen->ngay_bd  && Carbon::now() < $phanquen->ngay_kt){
                        return view('admin.project.Importdata2.ckdtdsv')->with([
                            'loai_dv'           => $loai_dv,
                            'donvi'             => $donvi,
                            'getFile'           => $getFile
                        ]);
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '32')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Congkhaidtdsv'), $picName);

            $data = [
                'type_excel'    => '32',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Congkhaidtdsv/'.$picName,
    ...
    public function importDataUnit(Request $req) {
        ...
        foreach($data as $dt){
            if($dt->ten != "" && $dt->tyle != ""){
                ...
                DB::table("excel_import_dientich_sv")->insert($dataInport);
            }
        }
        $respon = [
            'mes'   => 'done'
    ...
    public function exportCkdtdsv() {
        return Excel::download(new CkdtdsvExport, 'Cong-khai-dien-tich-dat-sinh-vien.xlsx');
    }


    public function dataUnit(Request $req){
    	$donviExcel = DB::table("excel_import_dientich_sv AS ckdtdsv");
        if(isset($req->id) && $req->id != ''){
            $donviExcel = $donviExcel->where("ckdtdsv.id", $req->id)->first();
            return json_encode($donviExcel);
        }else{
	        $donviExcel = $donviExcel
	                ->select('ckdtdsv.id', 'ckdtdsv.ten', 'ckdtdsv.ty_le'
	                 );
    ...
    public function deleteUnit(Request $req){
        DB::table('excel_import_dientich_sv')->where("id", $req->id_delete)->delete();
        return back()->with('success', 
    ...
    public function updateUnit(Request $req){
        ...
        DB::table("excel_import_dientich_sv")->where("id", $req->id_unit)
        		->update($data);
        return back()->with('success', 
