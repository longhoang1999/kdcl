...
{{-- Page title --}}
@section('title')
    @lang('project/ImportdataExcel/title.hnhtkh')
    @parent
@stop
...
@section('content')
    <section class="content indexpage pr-3 pl-3">
        ...
        <section class="content-body">
            <div class="form-standard">
                <div class="item-group-button right-block mb-2">
                    <button href="" class="btn btn-benchmark mr-2" type="button" data-toggle="modal"
                        data-target="#modal_unit" data-bs-toggle="tooltip" data-bs-placement="top"
                        title="@lang('project/ImportdataExcel/title.nhap_excel')">
                        <i class="bi bi-file-earmark-arrow-up" style="font-size: 35px;color: #50cd89;"></i>
                    </button>
                    <a href="{{ route('admin.importdata.ckhnhtkh.exportCkhnhtkh') }}" class="btn btn-benchmark mr-2"
                        type="button" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/Selfassessment/title.xuat_excel')">
                        <i class="bi bi-file-earmark-excel " style="font-size: 35px;color: #50cd89;"></i>
                    </a>
                    <button class="btn" data-toggle="modal" data-target="#modalDeleteAll__" data-nametable="excel_import_hn_htkh" data-bs-toggle="tooltip" data-bs-placement="top" title="@lang('project/ImportdataExcel/title.xoatatca')">
                ...
                <table class="table table-striped table-bordered" id="table" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th>
                                @lang('project/ImportdataExcel/title.tenchude')
                            </th>
                            <th>
                                @lang('project/ImportdataExcel/title.tgtc')
                            </th>
                            <th>
                                @lang('project/ImportdataExcel/title.diadiemtc')
                            </th>
                            <th>
                                @lang('project/ImportdataExcel/title.sldbtd')
                            </th>
                            <th>
        ...
        <!-- Import modal excel -->
        <div class="modal fade" id="modal_unit" tabindex="-1" role="dialog" aria-labelledby="modalUnitLabel"
            aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalUpdateLabel">
                            @lang('project/ImportdataExcel/title.cntthnht')
                        </h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    ...
                    <div class="modal-body">
                        <form action="{{ route('admin.importdata.ckhnhtkh.updateUnit') }}" method="post"
                            id="update-unit">
                            @csrf
                            ...
                            <div class="container-fuild">
                                <div class="row">
                                    <div class="form-group col-md-4">
                                        <label for="fortenchude">
                                            <span>@lang('project/ImportdataExcel/title.tenchude')</span>
                                        </label>
                                        <input type="text" class="form-control " id="fortenchude"
                                            placeholder="@lang('project/ImportdataExcel/title.tenchude')" name="tenchude">
                                    </div>
                                    <div class="form-group col-md-2">
                                        <label for="fortgtc">
                                            <span>@lang('project/ImportdataExcel/title.tgtc')</span>
                                        </label>
                                        <input type="date" class="form-control " id="fortgtc"
                                            placeholder="@lang('project/ImportdataExcel/title.tgtc')" name="tgtc">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="fordiadiemtc">
                                            <span>@lang('project/ImportdataExcel/title.diadiemtc')</span>
                                        </label>
                                        <input type="text" class="form-control " id="fordiadiemtc"
                                            placeholder="@lang('project/ImportdataExcel/title.diadiemtc')" name="diadiemtc">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="forsldbtd">
                                            <span>@lang('project/ImportdataExcel/title.sldbtd')</span>
                                        </label>
                                        <input type="number" class="form-control " id="forsldbtd"
                                            placeholder="@lang('project/ImportdataExcel/title.sldbtd')" name="sldbtd">
                                    </div>
    ...
    <script>
        ...
        $(function() {
            table = $('#table').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{!! route('admin.importdata.ckhnhtkh.dataUnit') !!}",
                columns: [
        ...
        $('#import_unit').on('click', function() {
            ...
            $.ajax({
                url: "{!! route('admin.importdata.ckhnhtkh.importUnit') !!}",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        //     return `${ date_info.getDate() < 10 ? '0' + date_info.getDate() : date_info.getDate() }-${ (date_info.getMonth() + 1) < 10 ? '0' + (date_info.getMonth() + 1) : (date_info.getMonth() + 1) }-${ date_info.getFullYear() }`;
        // }
        ...
        $("#import_unit_data").click(function() {
            if (checkEmpty() && checkWebsite() && checkEmail() && checkPhone() &&
                ...
                $(".row_number").each(function(index) {
                    let dataObj = {
                        'tenchude': $(this).find('.row1').text().trim(),
                        'tgtc': $(this).find('.row2').text().trim(),
                        'diadiemtc': $(this).find('.row3').text().trim(),
                        'sldbtd': $(this).find('.row4').text().trim(),

                    }
                    dataSubmit.push(dataObj);
                });

                let loadData = "{{ route('admin.importdata.ckhnhtkh.importDataUnit') }}";
                fetch(loadData, {
                        headers: {
        ...
        $('#modalDelete').on('show.bs.modal', function(event) {
            ...
            var route = "{{ route('admin.importdata.ckhnhtkh.deleteUnit') }}" + "?id_delete=" + id;
            var modal = $(this);
        ...
        $('#modalUpdate').on('show.bs.modal', function(event) {
            ...
            var id = button.data('id')
            $("#id_unit").val(id);
            let loadData = "{{ route('admin.importdata.ckhnhtkh.dataUnit') }}" + "?id=" + id;
            fetch(loadData, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                .then((response) => response.json())
                .then((data) => {
                    $("#fortenchude").val(data.tcd);
                    $("#fortgtc").val(data.tgtc);
                    $("#fordiadiemtc").val(data.ddtc);
                    $("#forsldbtd").val(data.so_luong);
