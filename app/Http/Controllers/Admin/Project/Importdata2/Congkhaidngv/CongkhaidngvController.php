<?php namespace App\Http\Controllers\Admin\Project\Importdata2\Congkhaidngv;
...
class CongkhaidngvController extends DefinedController{
    public function index(){
		...
		$donvi = DB::table("donvi")->select("id", "ten_donvi", "deleted_at","loai_dv_id")
                ...
                ->get();
        $getFile = DB::table('excel_import_data2')->where('type_excel', '39')->select("id", "year")->get();
		
        return view('admin.project.Importdata2.ckdngvkn')->with([
            'loai_dv'           => $loai_dv,
            'donvi'             => $donvi,
            'getFile'           => $getFile
        ]);
    ...
    public function addfilenew(Request $req) {
        if($req->file('file') != null){
            $check = DB::table('excel_import_data2')->where("type_excel", '39')
                        ->where("year", $req->year);
            if($check->count() > 0){
            ...
            $image->move(public_path('UpdateExcelFile/Congkhaidngv'), $picName);

            $data = [
                'type_excel'    => '39',
                'year'  => $req->year,
                'url'   => 'UpdateExcelFile/Congkhaidngv/'.$picName,
